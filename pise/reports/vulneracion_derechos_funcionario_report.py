...
class ReportVulneracionDerechosFuncionarios():

    def __init__(self, caso:models.VulneracionDerechosFuncionarios=None, request=None ) -> None:
        self.caso = caso
    ...
    def create_pdf(self):
        ...
        response['Content-Disposition'] = 'filename="vulneracion_funcionarios_nombre"'

        pdf = SimpleDocTemplate(response, pagesize=A4, rightMargin=72, leftMargin=72, topMargin=50, bottomMargin=0)
        styles = getSampleStyleSheet()
        elemWidth = 500
        titleTable = Table([
            [Paragraph("<para align=center>Caso vulneracion_funcionarios Escolar</para>", styles['Heading1'])]
        ], elemWidth)
        ...
        alumno_data = Table([
            ["Nombre: ", self.caso.funcionario_afectado, "Rut: " , rut],
        ...
        tipo_caso = Table([
            ["Tipo de Caso: ", self.caso.get_tipo_vulneracion_display()],
        ...
        pdf.title = "Caso Vulneración de Derechos Funcionario Escolar"
        pdf.build(Story)
