...
class ReporteDiscriminacion():

    def __init__(self, caso:models.Discriminacion=None, request=None ) -> None:
        self.caso = caso
    ...
    def create_pdf(self):
        ...
        response['Content-Disposition'] = 'filename="discriminacion_nombre"'

        pdf = SimpleDocTemplate(response, pagesize=A4, rightMargin=72, leftMargin=72, topMargin=50, bottomMargin=0)
        styles = getSampleStyleSheet()
        elemWidth = 500
        titleTable = Table([
            [Paragraph("<para align=center>Caso Discriminacion Escolar</para>", styles['Heading1'])]
        ], elemWidth)
        ...
        tipo_caso = Table([
            ["Tipo de Caso: ", self.caso.get_tipo_discrim_display()],
        ...
        pdf.title = "Caso Discriminación Escolar"
        pdf.build(Story)
