...
class ReporteConnotacionSexual():

    def __init__(self, caso:models.ConnotacionSexual=None, request=None ) -> None:
        self.caso = caso
    ...
    def create_pdf(self):
        ...
        response['Content-Disposition'] = 'filename="connotacion_nombre"'

        pdf = SimpleDocTemplate(response, pagesize=A4, rightMargin=72, leftMargin=72, topMargin=50, bottomMargin=0)
        styles = getSampleStyleSheet()
        elemWidth = 500
        titleTable = Table([
            [Paragraph("<para align=center>Caso Connotacion Sexual</para>", styles['Heading1'])]
        ], elemWidth)
        ...
        tipo_caso = Table([
            ["Tipo de Caso: ", self.caso.get_tipo_connotacion_sexual_display()],
            ["Fecha del Caso: ", self.caso.fecha_creacion.strftime('%d/%d/%Y')],
        ], 100)
        ...
        pdf.title = "Caso Connotacion Sexual Escolar"
        pdf.build(Story)
